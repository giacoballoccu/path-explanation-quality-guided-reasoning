...
class BatchKGEnvironment(object):
    def __init__(self, dataset_str, max_acts, max_path_len=3, state_history=1, optimize_for=None, alpha=None):
        ...
        self.explainability_reward = optimize_for
        self.alpha = alpha
        ...
        for pattern_id in valid_patterns:
            ...
            if pattern_id == 1: #Len 3 must be normalized to 4
        ...
        # Exp metri optimization
        if self.explainability_reward == "LIR":
            self.word_idxs = None
            if self.dataset_name not in AMAZON_DATASETS:
                self.item_idxs, self.user2timestamp = get_interaction2timestamp_map(self.dataset_name)
            else:
                self.item_idxs, self.word_idxs, self.user2timestamp = get_interaction2timestamp_with_words(
                    self.dataset_name)
            self.LIR_matrix, self.LIR_matrix_words = generate_LIR_matrix(dataset_str, self.user2timestamp, self.item_idxs,
                                                                         self.word_idxs)

        if self.explainability_reward == "SEP":
            self.SEP_matrix = generate_SEP_matrix(self.dataset_name)

        if self.explainability_reward == "ETD":
            self.candidates_per_ptype = {path_type: 0 for path_type in KG_RELATION[dataset_str][main_entity]}
            self.MAX_PATH_TYPES = len(set([x[-1][0] for x in PATH_PATTERN[self.dataset_name].values()]))
            self.uid_type = {}
            self.uid_type_score = {}
    ...
    def _get_actions(self, path, done):
        ...
        if self.explainability_reward == "ETD":
            if uid not in self.uid_type:
                self.uid_type[uid] = set()
            if uid not in self.uid_type_score:
                self.uid_type_score[uid] = {}
        ...
        for r, next_node_id in candidate_acts:
            ...
            #    score = 99999.0
            if len(path) == 1 and self.explainability_reward == "LIR":
                uid = path[0][-1]
                li = next_node_id
                if r == WATCHED:
                    score += self.LIR_matrix[uid][li]
                if r == MENTION:
                    score = (1-self.alpha) * score + (self.alpha * self.LIR_matrix_words[uid][li])
            if len(path) == 2 and self.explainability_reward == "SEP":
                se_type = RELATION_TO_ENTITY[self.dataset_name][r]
                se = next_node_id
                if se not in self.SEP_matrix[se_type]:
                    se = 0.0
                score = (1-self.alpha) * score + (self.alpha * self.SEP_matrix[se_type][se]) #weighted sum 0.5
            if len(path) == 3 and self.explainability_reward == "ETD":
                ptype = r
                uid = path[0][-1]
                if r not in self.uid_type_score[uid]:
                    self.uid_type_score[uid][r] = score
                if r not in self.uid_type[uid]:
                    score += self.alpha
                elif r in self.uid_type[uid] and score > self.uid_type_score[uid][ptype]:
                    self.uid_type_score[uid][r] = score
                    score += self.alpha
            scores.append(score)
    ...
    def _get_reward(self, path):
        ...
        if curr_node_type == main_entity:
            ...
            # Give exp reward based on the property of the path
            bonus = 0
            if self.explainability_reward == "ETD":
                if uid not in self.uid_type:
                    self.uid_type[uid] = set()
                if relation not in self.uid_type[uid]:
                    bonus += 1
                    self.uid_type[uid].add(relation)

            if self.explainability_reward == "LIR":
                linked_interaction = path[1][-1]
                interaction_type = path[1][0]
                lir = 0
                if interaction_type == WATCHED:
                    lir = self.LIR_matrix[uid][linked_interaction]
                elif interaction_type == MENTION:
                    lir = self.LIR_matrix_words[uid][linked_interaction]
                bonus = lir

            if self.explainability_reward == "SEP":
                bonus = self.SEP_matrix[shared_entity][eid]
            score = (np.dot(u_vec, p_vec) / self.u_p_scales[uid])


            target_score = ((1-self.alpha) * max(0, score)) + (self.alpha * bonus)
            #target_score = score
