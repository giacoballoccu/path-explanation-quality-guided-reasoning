...
from utils import *
from data_utils import DataLoader, AmazonDataLoader
from transe_model import KnowledgeEmbedding


logger = None

def train_with_words(args):
...
def extract_embeddings(args):
    ...
    if dataset_name == "ml1m":
        ...
        embeds = extract_embeddings_amazon(state_dict)
    else:
...
def extract_embeddings_amazon(state_dict):
    embeds = {
        USER: state_dict['user.weight'].cpu().data.numpy()[:-1],  # Must remove last dummy 'user' with 0 embed.
        ...
        CATEGORY: state_dict['category.weight'].cpu().data.numpy()[:-1],
        RPRODUCT: state_dict['rproduct.weight'].cpu().data.numpy()[:-1],
