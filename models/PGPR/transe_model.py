...
class KnowledgeEmbedding(nn.Module):
    def __init__(self, dataset, args):
        ...
        elif self.dataset_name in [CLOTH, CELL, CD, BEAUTY]:
    ...
    def initialize_entity_embeddings_amazon(self, dataset):
        self.entities = edict(
            ...
            rproduct=edict(vocab_size=dataset.rproduct.vocab_size),
    ...
    def compute_loss_lastfm(self, batch_idxs):
        ...
        srs2_loss, srs2_embeds = self.neg_loss('song', 'original_version_of', 'related_song', song_idxs,
                                               related_song2_idxs)
        ...
        srs3_loss, srs3_embeds = self.neg_loss('song', 'alternative_version_of', 'related_song', song_idxs,
                                               related_song3_idxs)
    ...
    def compute_loss_amazon(self, batch_idxs):
        ...
        rproduct1_idxs = batch_idxs[:, 5] #also_bought
        rproduct2_idxs = batch_idxs[:, 6] #also_viewed
        rproduct3_idxs = batch_idxs[:, 6] #bought_together
        ...
        pr1_loss, pr1_embeds = self.neg_loss('product', 'also_bought', 'rproduct', product_idxs, rproduct1_idxs)
        if pr1_loss is not None:
            regularizations.extend(pr1_embeds)
            loss += pr1_loss

        # product + also_viewed -> related_product2
        pr2_loss, pr2_embeds = self.neg_loss('product', 'also_viewed', 'rproduct', product_idxs, rproduct2_idxs)
        if pr2_loss is not None:
            regularizations.extend(pr2_embeds)
            loss += pr2_loss

        #product + bought_together -> related_product3
        pr3_loss, pr3_embeds = self.neg_loss('product', 'bought_together', 'rproduct', product_idxs, rproduct3_idxs)
    ...
    def compute_loss(self, batch_idxs):
        ...
        if self.dataset_name == ML1M:
            ...
            return self.compute_loss_beauty(batch_idxs) #^^^
        elif self.dataset_name in AMAZON_DATASETS:
