...
def train(args):
    env = BatchKGEnvironment(args.dataset, args.max_acts, max_path_len=args.max_path_len, state_history=args.state_history, optimize_for=args.metric, alpha=args.alpha)
    uids = list(env.kg(USER).keys())
    dataloader = ACDataLoader(uids, args.batch_size)
    pretrain_sd = torch.load("./tmp/lastfm/train_agent/policy_model_epoch_20.ckpt")
...
def main():
    ...
    parser.add_argument('--metric', type=str, default=None, help='Explanation metric to optimize in processing [LIR,SEP,ETD]')
    parser.add_argument('--alpha', type=str, default=None, help='How much importance to give to explanation property 0 and 1')
