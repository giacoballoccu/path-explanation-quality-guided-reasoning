...
if __name__ == '__main__':
    ...
    parser.add_argument('--opt', type=str, default="ETDopt", help='One of ["softETD", "softSEP", "softLIR", "ETDopt", "SEPopt", "LIRopt", "ETD_SEP_opt", "ETD_LIR_opt", "SEP_LIR_opt", "ETD_SEP_LIR_opt"]')
    ...
    parser.add_argument('--save_after_exp_quality_distributions', type=bool, default=False, help='If true save a csv with the distribution of after-opt values for the exp metrics and groups')
    ...
    if args.eval_baseline == True:
        ...
        print_expquality_metrics(path_data.dataset_name, path_data.sens_attribute_flags, exp_metrics_before["LIR"],
        ...
        for chosen_optimization in alpha_optimizations:
            ...
            for alpha in alphas:
                ...
                if chosen_optimization == "ETDopt":
                    weighted_opt_ETD(path_data, alpha)
                elif chosen_optimization == "SEPopt":
                    weighted_opt_SEP(path_data, alpha)
                elif chosen_optimization == "LIRopt":
                    weighted_opt_LIR(path_data, alpha)
                elif chosen_optimization == "ETD_SEP_opt":
                    weighted_opt_ETD_SEP(path_data, alpha)
                elif chosen_optimization == "ETD_LIR_opt":
                    weighted_opt_ETD_LIR(path_data, alpha)
                elif chosen_optimization == "SEP_LIR_opt":
                    weighted_opt_LIR_SEP(path_data, alpha)
                elif chosen_optimization == "ETD_SEP_LIR_opt":
                    weighted_opt_ETD_SEP_LIR(path_data, alpha)
                ...
                print_expquality_metrics(path_data.dataset_name, path_data.sens_attribute_flags, exp_metrics_after["LIR"],
